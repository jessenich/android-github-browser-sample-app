...
class ApiResponseTest {
    ...
    fun link() {
        val link =
            "<https://api.github.io/search/repositories?q=foo&page=2>; rel=\"next\"," +
                    " <https://api.github.io/search/repositories?q=foo&page=34>; rel=\"last\""
    ...
    @Test
    fun badPageNumber() {
        val link = "<https://api.github.io/search/repositories?q=foo&page=dsa>; rel=\"next\""
        val headers = okhttp3.Headers.of("link", link)
        val response = ApiResponse.create<String>(Response.success("foo", headers))
    ...
    @Test
    fun badLinkHeader() {
        val link = "<https://api.github.io/search/repositories?q=foo&page=dsa>; relx=\"next\""
        val headers = okhttp3.Headers.of("link", link)
        val response = ApiResponse.create<String>(Response.success("foo", headers))
        assertThat<Int>((response as ApiSuccessResponse).nextPage, nullValue())
    }
